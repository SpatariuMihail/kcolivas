...
#define PRE_OPENCORE_FREQ   8
    ...
    /*
    call sample:   if called outside of read_temp_func , then we need use opencore_readtemp_mutex
                      if called inside of read_temp_func, just call bitmain_reinit(); only !!!
    pthread_mutex_lock(&opencore_readtemp_mutex);
    bitmain_reinit();
    pthread_mutex_unlock(&opencore_readtemp_mutex);
    ...
    */
	...
	set_Hardware_version(0x80000000);
        ...
        for(i=0; i < BITMAIN_MAX_CHAIN_NUM; i++)
        {
            if(dev->chain_exist[i] == 1)
            {
#ifndef ENABLE_HIGH_VOLTAGE_OPENCORE    // if use high voltage open core, the voltage has been set by set_voltage_T9_18_into_PIC
                set_pic_voltage(i, 0);  // the second parameter is not used, we set 0 for T9_18
#endif
                enable_pic_dac(i);
            }
            ...
            if(opt_pre_heat)
            {
                ...
                //if(readRebootTestNum()<=0)        // we will do preheat every time, even in reboot test mode.
                {
                    ...
                    //if(!test_result)
                    {
                        ...
                        // clement debug to show ticket mask!!!
                        check_asic_reg(TICKET_MASK);
